...
const TreeIcon = ({ expanded, data }: MarsTreeNodeAttribute) => {
  if ((!data.children || data.children.length === 0) && !data.group) {
    return <MarsIcon icon="split" width="14" theme="multi-color" fill={["#FFF", "#43CCF8", "#43CCF8", "#43CCF8"]}></MarsIcon>
  } else if (!expanded) {
    return <MarsIcon icon="folder-close" width={14} color="#db9829" theme="filled"></MarsIcon>
  } else if (expanded) {
